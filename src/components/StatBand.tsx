        ...
        >
          {copy.stats.items.map((stat, i) => (
            <motion.div key={stat.label} variants={fadeUp} className="flex flex-col items-start">
              <div
                className="font-display text-[3.6rem] leading-none tracking-[-0.04em] md:text-[5rem]"
                style={{
              ...
              />
              <div className="mt-3 font-mono text-[11px] uppercase tracking-[0.26em] text-ink-800">
                {stat.label}
              </div>
            </motion.div>
          ))}
